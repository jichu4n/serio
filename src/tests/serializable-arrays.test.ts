...
describe('SArrayWithWrapper', function () {
  test('create', function () {
    const arr1 = SArray.as(SUInt16BE).ofLength(3, 42);
    expect(arr1.value).toStrictEqual([42, 42, 42]);

    const arr2 = SArray.as(SUInt16BE).ofLength(3, function () {
      return 42;
    });
    expect(arr2.value).toStrictEqual([42, 42, 42]);

    const arr3 = SArray.as(SUInt16BE).ofLength(3, () => 42);
    expect(arr3.value).toStrictEqual([42, 42, 42]);
  });

  test('serialize and deserialize', function () {
    const arr1 = SArray.as(SUInt16BE).of([100, 200, 300]);
    ...
    const arr3 = SArray.as(SUInt16BE).ofLength(3, 0);
    arr3.deserialize(arr1.serialize());
    expect(arr3.value).toStrictEqual([100, 200, 300]);

    const arr4 = SArray.as(SArray.as(SUInt16BE)).of([
      [1, 2, 3],
      [4, 5, 6],
      [7, 8, 9],
    ]);
    expect(arr4.getSerializedLength()).toStrictEqual(18);
    const arr5 = SArray.as(SUInt16BE).ofLength(9, 0);
  ...
  test('toJSON', function () {
    ...
    const arr2 = SArray.as(SStringNT).of(['hello', 'world']);
